...
class NLIModel:
    ...
    def check_goal(self, goal: str, plan_goal: str):
        """
        Infer whether the plan goal and agent goal is the same
        :param goal: agent goal
        :param plan_goal: plan goal
        :return: True whether both goals are the same
        """
        pair = (goal, plan_goal)
        logits = self._predict_nli([pair])
        return logits.argmax(-1) == self.entailment_idx
